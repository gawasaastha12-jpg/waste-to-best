...
Follow these steps to run the WasteTrack+ development server locally:

### 1. Prerequisites
Ensure you have Python 3.11+ and Redis installed and running on your local machine.
...
# Optional: Set to True to bypass Redis cache/Celery locally
USE_LOCAL_CACHE=False 
```

### 4. Database Setup & Migrations
Run the migrations to set up database schemas (defaults to a local SQLite database if `DATABASE_URL` is omitted from `.env`):
```bash
...
Start the Celery background worker (in a separate terminal window, with virtualenv active):
* **Windows:**
  ```powershell
...
## 🧪 Running Tests
To run the automated Django test suite using SQLite locally:
```bash
# Clear DATABASE_URL temporarily to force SQLite tests
$env:DATABASE_URL=""
...
When deploying containerized to platforms like Render:
1. Ensure your container includes system dependencies for spatial backends (`binutils`, `libproj-dev`, `gdal-bin`, `libmagic1`).
2. Map environment variables in the Render dashboard:
   - `DEBUG=False`
